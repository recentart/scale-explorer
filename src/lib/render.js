...
function drawShape(it, box, groundY, cls) {
  ...
  return `<g class="${cls}" transform="translate(${n3(x)} ${n3(y)}) scale(${box.sx} ${box.sy})">${shapeLayers(shape)}</g>`
}
...
export function renderBars(objects, { system = 'metric', highlight } = {}) {
  const rows = objects.map(o => ({ o, r: measureRange(o.measures[o.headline]) }))
  const max = Math.max(...rows.map(x => x.r.hi))
  const color = o => shapeOf(o.profile.shape).layers[0].fill
  return `<ol class="bars">` + rows.map(({ o, r }) => {
    const lo = (r.lo / max) * 1000, hi = (r.hi / max) * 1000
    const vis = Math.max(lo, 3)
    const tiny = hi < 3
    return `<li class="bar-row${o.slug === highlight ? ' is-highlight' : ''}">` +
      `<span class="bar-name">${o.slug === 'custom' ? esc(o.name) : `<a href="/objects/${esc(o.slug)}">${esc(o.name)}</a>`}</span>` +
      `<span class="bar-dim">${esc(headlineLabel(o))}</span>` +
      `<span class="bar-track"><svg viewBox="0 0 1000 16" preserveAspectRatio="none" aria-hidden="true" focusable="false">` +
      (hi > lo + 0.5 ? `<rect class="bar-range" x="0" y="2" width="${n1(Math.max(hi, 3))}" height="12" fill="${color(o)}"/>` : '') +
      `<rect class="bar-fill" x="0" y="2" width="${n1(vis)}" height="12" fill="${color(o)}"/></svg></span>` +
      `<span class="bar-value">${esc(headlineText(o, system))}${tiny ? ' <span class="bar-note">(too short to show at this scale)</span>' : ''}</span></li>`
  }).join('') + `</ol>`
}
