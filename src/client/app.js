...
import { vizFigure, zoomFigures, ladderSection, tinyNote } from '../lib/figure.js'
import { sizeOf, shapeOf } from '../lib/layout.js'
import { compareSentence, comparisonSet, phraseOf } from '../lib/compare.js'
...
function hydrateFigure(fig) {
  ...
  const host = fig
  const run = watch(host, w => draw(Math.round(stageEl.clientWidth || barsEl?.clientWidth || w)))
...
function initComparePage() {
  ...
  const optionsHtml = selA.innerHTML
  ...
  function update() {
    ...
    const lineup = [...(withHuman ? [BY.get('human')] : []), ...chosen].sort((a, b) => sizeOf(a) - sizeOf(b))
