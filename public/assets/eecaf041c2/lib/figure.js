...
import { esc, renderStage, renderBars, ladderSteps, volumeCubes } from './render.js'
...
export function vizFigure(objects, {
  ...
  const tabHtml = tabs ? `<div class="viz-tabs" role="tablist" aria-label="View">
      ...
      <button type="button" role="tab" id="${id}-tab-mass" aria-selected="false" aria-controls="${id}-mass" tabindex="-1">Weight</button>
      <button type="button" role="tab" id="${id}-tab-volume" aria-selected="false" aria-controls="${id}-volume" tabindex="-1">Volume</button>
    </div>` : ''
  ...
  <div class="viz-pane viz-mass" id="${id}-mass" role="tabpanel" aria-labelledby="${id}-tab-mass" hidden>${massPane(objects, { highlight, system })}</div>
  <div class="viz-pane viz-volume" id="${id}-volume" role="tabpanel" aria-labelledby="${id}-tab-volume" hidden>${volumePane(objects, { highlight, system, width, id })}</div>` : ''}
...
export function massPane(objects, { highlight, system = 'metric' } = {}) {
  return `<p class="viz-pane-note">Weight (mass) on one linear scale.</p>${renderBars(objects, { highlight, system, kind: 'mass' })}`
}

const sideOf = c => c.measures.side.max ?? c.measures.side.value

export function volumePane(objects, { highlight, system = 'metric', width = 960, id = 'v' } = {}) {
  const cubes = volumeCubes(objects)
  const missing = objects.filter(o => !cubes.some(c => c.slug === o.slug) && o.slug !== 'custom' && o.slug !== 'you')
  const note = missing.length ? `<p class="viz-missing">No sourced volume for ${esc(missing.map(o => o.name).join(', '))}.</p>` : ''
  if (!cubes.length) return `<p class="viz-empty">None of these objects has a sourced volume.</p>`
  const { svg } = renderStage(cubes.sort((a, b) => sideOf(a) - sideOf(b)), { width, maxHeight: 360, minHeight: 160, highlight, system, idPrefix: `${id}-vol` })
  return `<p class="viz-pane-note">Each square is the side of a cube holding the same volume, all at one scale.</p><div class="viz-stage">${svg}</div>${note}`
}

// Cards for "how many fit" rows: a big number, what is counted, and what it matches.
export function howManyHtml(rows) {
  if (!rows.length) return ''
  const verb = { length: 'match the', mass: 'weigh as much as', volume: 'fill' }
  return `<ul class="fit-cards">${rows.map(r => `<li class="fit-card fit-${r.kind}">
    <span class="fit-count">≈ ${esc(r.count)}</span>
    <span class="fit-what">× ${esc(r.smallLabel)}</span>
    <span class="fit-how">${esc(r.what)} ${esc(verb[r.kind])}</span>
    <span class="fit-big">${esc(r.bigLabel)}</span>
  </li>`).join('')}</ul>`
}
