...
export const UNITS = {
  ...
  // Very small and very large lengths
  pm: { kind: 'length', base: 1e-12, symbol: 'pm', system: 'metric' },
  nm: { kind: 'length', base: 1e-9, symbol: 'nm', system: 'metric' },
  um: { kind: 'length', base: 1e-6, symbol: 'µm', system: 'metric' },
  au: { kind: 'length', base: 149597870700, symbol: 'AU', system: 'metric' },
  ly: { kind: 'length', base: 9460730472580800, symbol: 'light-years', system: 'metric' },
  pc: { kind: 'length', base: 3.0856775814913673e16, symbol: 'parsecs', system: 'metric' },
  solarRadius: { kind: 'length', base: 695700000, symbol: 'solar radii', system: 'metric' },
  // Small masses
  g: { kind: 'mass', base: 0.001, symbol: 'g', system: 'metric' },
  mg: { kind: 'mass', base: 1e-6, symbol: 'mg', system: 'metric' },
  oz: { kind: 'mass', base: 0.028349523125, symbol: 'oz', system: 'imperial' },
  // Volumes (base: cubic metres)
  m3: { kind: 'volume', base: 1, symbol: 'm³', system: 'metric' },
  cm3: { kind: 'volume', base: 1e-6, symbol: 'cm³', system: 'metric' },
  L: { kind: 'volume', base: 0.001, symbol: 'L', system: 'metric' },
  mL: { kind: 'volume', base: 1e-6, symbol: 'mL', system: 'metric' },
  km3: { kind: 'volume', base: 1e9, symbol: 'km³', system: 'metric' },
  ft3: { kind: 'volume', base: 0.028316846592, symbol: 'cu ft', system: 'imperial' },
  yd3: { kind: 'volume', base: 0.764554857984, symbol: 'cu yd', system: 'imperial' },
  galUS: { kind: 'volume', base: 0.003785411784, symbol: 'US gal', system: 'imperial' },
  flozUS: { kind: 'volume', base: 0.0000295735295625, symbol: 'US fl oz', system: 'imperial' },
  mi3: { kind: 'volume', base: 4168181825.440579, symbol: 'cu mi', system: 'imperial' },
}
...
export function pickUnit(kind, baseValue, system) {
  ...
  if (kind === 'length') {
    // Microscopic and astronomical sizes use the same units in both systems.
    if (v < 1e-9) return 'pm'
    if (v < 1e-6) return 'nm'
    if (v < 1e-3 && (!imperial || v < 2.54e-5)) return 'um'
    if (v >= 9.4607304725808e14) return 'ly'
    if (imperial) return v < 0.3048 ? 'in' : v < 16093.44 ? 'ft' : 'mi'
    return v < 0.01 ? 'mm' : v < 1 ? 'cm' : v < 10000 ? 'm' : 'km'
  }
  if (kind === 'mass') {
  ...
  if (kind === 'volume') {
    if (imperial) return v < 0.0283 ? 'flozUS' : v < 4.17e9 ? 'ft3' : 'mi3'
    return v < 0.001 ? 'mL' : v < 1 ? 'L' : v < 1e9 ? 'm3' : 'km3'
  }
...
export const INPUT_MIN_M = 1e-9
export const INPUT_MAX_M = 1e22

// Units a visitor can type besides the four in the unit menu.
export const TYPED_UNITS = ['nm', 'um', 'mm', 'cm', 'in', 'ly']

const UNIT_WORDS = {
  nm: 'nm', nanometer: 'nm', nanometers: 'nm', nanometre: 'nm', nanometres: 'nm',
  um: 'um', 'µm': 'um', micron: 'um', microns: 'um', micrometer: 'um', micrometers: 'um', micrometre: 'um', micrometres: 'um',
...
function validated(meters, value, unit) {
  ...
  if (meters < INPUT_MIN_M) return { ok: false, error: 'That is smaller than 1 nanometre. Try a larger size.' }
  if (meters > INPUT_MAX_M) return { ok: false, error: 'That is larger than about a million light-years. Try a smaller size.' }
...
export function parseLength(text, fallbackUnit = 'm') {
  ...
  if (fi) {
    ...
    return validated((ft * 12 + inch) * 0.0254, ft * 12 + inch, 'in')
  }

  const m = raw.replace(/light[\s-]+years?/, 'ly').match(/^([+-]?[\d.,]*\d[\d.,]*(?:e[+-]?\d+)?)\s*([a-zµ'’′"”]+)?\.?$/)
  if (!m) {
    if (/^[+-]?[\d.,]/.test(raw)) {
  ...
  if (!INPUT_UNITS.includes(unit) && !TYPED_UNITS.includes(unit)) return { ok: false, error: 'Choose meters, feet, kilometers or miles.' }
