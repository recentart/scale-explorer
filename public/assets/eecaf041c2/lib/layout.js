...
export const BUILTIN_SHAPES = {
  ...
  'volume-cube': {
    w: 100, h: 100,
    layers: [
      { d: 'M0 100 V0 H100 V100 Z', fill: '#7a8aa0' },
      { d: 'M0 0 H100 V8 H8 V100 H0 Z', fill: '#ffffff', opacity: 0.18 },
    ],
  },
  'custom-span': {
    w: 100, h: 4,
...
export function drawingBox(obj, end = 'hi') {
  ...
  const pick = key => {
    ...
    // Balls are specified by circumference; the drawing needs the diameter.
    const k = obj.profile.circumference ? 1 / Math.PI : 1
    return (end === 'lo' ? r.lo : end === 'mid' ? midOf(r) : r.hi) * k
  }
...
export function layoutStage(objects, {
  ...
  if (n > 1 && innerW - baseGap * (n - 1) < n * slotMin) {
    baseGap = Math.max(0, (innerW - n * slotMin) / (n - 1))
    if (innerW - baseGap * (n - 1) < n * slotMin) slotMin = (innerW - baseGap * (n - 1)) / n
  }
...
export function pickReferences(obj, all, max = 2) {
  ...
  // Nothing familiar within reach (a galaxy): use the largest familiar smaller thing.
  if (!chosen.length) {
    const below = pool.filter(o => sizeOf(o) < size).sort((a, b) => sizeOf(b) - sizeOf(a))
    if (below.length) chosen.push(below[0])
  }
  return chosen
}
...
export function buildLadder(from, to, all, { maxRatio = 12, minRatio = 1.6, maxSteps = 30 } = {}) {
  let a = from, b = to
...
export function zoomLevels(objects, opts = {}, maxLevels = 8) {
  const levels = []
